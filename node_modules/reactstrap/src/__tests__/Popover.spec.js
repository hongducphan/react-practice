...
describe("Popover", () => {
  ...
  beforeEach(() => {
    ...
    element.innerHTML =
      '<p id="outerTarget">This is the popover <span id="innerTarget">target</span>.</p>';
  ...
  it("should allow custom classes to be added to the popover-inner", () => {
    const wrapper = mount(
      <Popover
        ...
        placement={placement}
        target="innerTarget"
        innerClassName="popover-special"
      >
        <PopoverHeader>Title</PopoverHeader>
        <PopoverBody>Content</PopoverBody>
      </Popover>
    ...
    expect(
      document
        .getElementsByClassName("popover-inner")[0]
        .className.indexOf("popover-special") > -1
    ).toBe(true);
  ...
  it("should not throw when passed a ref object as the target", () => {
    ...
    const wrapper = mount(
      <Popover target={targetObj} isOpen={isOpen} toggle={toggle}>
        Yo!
      </Popover>,
      { attachTo: container }
    );
