...
function value(body, params = [], async, generator) {
  const method = _core().types.objectMethod(
    "method",
...
function getSingleElementDefinition(path, superRef, classRef, file) {
  ...
  const properties = [
    ...
    isMethod
      ...
      : node.value
      ? value(_core().template.ast`{ return ${node.value} }`)
...
function getConstructorPath(path) {
  return path.get("body.body").find(path =>
    path.isClassMethod({
      kind: "constructor"
    })
  );
...
function insertInitializeInstanceElements(path, initializeInstanceId) {
  ...
  const initializeInstanceElements = _core().types.callExpression(
    ...
    if (isBase) {
      constructorPath
        ...
        .unshiftContainer("body", [
          _core().types.expressionStatement(initializeInstanceElements)
        ]);
    } else {
...
function transformClass(path, file) {
  ...
  const expr = _core().template.expression.ast`
      ${addDecorateHelper(file)}(
        ...
        function (${initializeId}, ${superClass ? superId : null}) {
          ${path.node}
          return { F: ${_core().types.cloneNode(
            path.node.id
          )}, d: ${definitions} };
        },
        ${superClass}
  ...
  return isDeclaration
    ? _core().template.ast`let ${path.node.id} = ${expr}`
    : expr;
}
