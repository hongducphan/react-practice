...
function read() {
  const u = peek();
  return (
    "\n" === u ? (line++, (column = 0)) : u ? (column += u.length) : column++,
    u && (pos += u.length),
    u
  );
}
const lexStates = {
  ...
  decimalPoint() {
    ...
    return util.isDigit(c)
      ? ((buffer += read()), void (lexState = "decimalFraction"))
...
function hexEscape() {
  ...
  if (((u += read()), (D = peek()), !util.isHexDigit(D)))
    throw invalidChar(read());
...
function invalidChar(u) {
  return syntaxError(
    void 0 === u
      ...
      : `JSON5: invalid character '${formatChar(u)}' at ${line}:${column}`
  );
}
function invalidEOF() {
...
function invalidIdentifier() {
  return syntaxError(
    `JSON5: invalid identifier character at ${line}:${(column -= 5)}`
  );
}
...
var stringify = function(u, D, e) {
  ...
  if (
    (null == D ||
      "object" != typeof D ||
      Array.isArray(D) ||
      ((e = D.space), (C = D.quote), (D = D.replacer)),
    "function" == typeof D)
  )
    F = D;
    ...
    for (const u of D) {
      ...
      "string" == typeof u
        ...
        : ("number" == typeof u ||
            ...
            u instanceof Number) &&
          (D = String(u)),
        void 0 !== D && t.indexOf(D) < 0 && t.push(D);
    }
  }
  return (
    e instanceof Number
  ...
  function i(u) {
    ...
    for (let e = D.length; e < u.length; e++)
      if (!util.isIdContinueChar(String.fromCodePoint(u.codePointAt(e))))
        return n(u);
    return u;
  }
};
