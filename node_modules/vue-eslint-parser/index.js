...
class LocationCalculator {
  constructor(gapOffsets, ltOffsets, baseOffset) {
    ...
    this.baseIndexOfGap =
      this.baseOffset === 0 ? 0 : sortedLastIndex(gapOffsets, this.baseOffset);
  }
  ...
  switch (newItems.length) {
    ...
    default:
      Array.prototype.splice.apply(
        items,
        [start, deleteCount].concat(newItems)
      );
      break;
  }
}
...
function parseAttributeValue(
  ...
  const result =
    quoted && node.value === ""
      ...
      : directiveName === "on"
      ? parseVOnExpression(node.value, locationCalculator, parserOptions)
...
function defineScopeAttributeVariable(
  code,
  parserOptions,
  locationCalculator,
  node
  ...
  StartTag(token) {
    ...
    if (namespace === NS.HTML) {
      if (
        ...
        const lang =
          (langAttr && langAttr.value && langAttr.value.value) || "html";
        if (lang !== "html") {
...
const entitySets = [
  { length: 32, entities: { "CounterClockwiseContourIntegral;": [8755] } },
  {
    length: 25,
    entities: {
  ...
  { length: 24, entities: { "NotNestedGreaterGreater;": [10914, 824] } },
  {
    length: 23,
    entities: {
...
class Tokenizer {
  ...
  isProvisionalState() {
    return (
      this.state.startsWith("RCDATA_") || this.state.startsWith("RAWTEXT_")
    );
  }
...
function tryParseSelector(rawSelector) {
  try {
    ...
    if (typeof err.offset === "number") {
      throw new Error(
        `Syntax error in selector "${rawSelector}" at position ${err.offset}: ${
          err.message
        }`
      );
    }
...
const forward = new CursorFactory(
  ForwardTokenCursor,
  ForwardTokenCommentCursor
);
const backward = new CursorFactory(
  BackwardTokenCursor,
