  ...
  create(context) {
      ...
      return context.report({
        ...
        fix(fixer) {
          return fixer.replaceTextRange(
            [initial.range[0], result.range[1]],
            name
          );
        }
      });
    ...
    function checkDestructured(node) {
      ...
      for (let i = 0; i < properties.length; i++) {
        ...
        if (
          (properties[i].key.type === "Identifier" &&
            properties[i].key.name === properties[i].value.name) ||
          (properties[i].key.type === "Literal" &&
            properties[i].key.value === properties[i].value.name)
        ) {
          reportError(
            properties[i],
            properties[i].key,
