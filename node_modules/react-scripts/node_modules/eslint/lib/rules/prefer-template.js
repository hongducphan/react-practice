...
function hasOctalEscapeSequence(node) {
  if (isConcatenation(node)) {
    return (
      hasOctalEscapeSequence(node.left) || hasOctalEscapeSequence(node.right)
    );
  }
...
module.exports = {
  ...
  create(context) {
    ...
    function getTemplateLiteral(currentNode, textBeforeNode, textAfterNode) {
      ...
      if (
        isConcatenation(currentNode) &&
        hasStringLiteral(currentNode) &&
      ...
      return `\`\${${textBeforeNode || ""}${sourceCode.getText(
        currentNode
      )}${textAfterNode || ""}}\``;
    }
