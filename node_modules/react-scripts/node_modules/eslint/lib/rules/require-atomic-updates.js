...
module.exports = {
  ...
  create(context) {
    ...
    function resolveVariableInScope(identifier, scope) {
      return (
        scope.variables.find(variable => variable.name === identifier.name) ||
    ...
    function resolveVariable(identifier) {
      if (!resolvedVariableCache.has(identifier)) {
        ...
        if (surroundingScope) {
          resolvedVariableCache.set(
            identifier,
            resolveVariableInScope(identifier, surroundingScope)
          );
        } else {
    ...
    function isLocalVariableWithoutEscape(expression, surroundingFunction) {
      ...
      return (
        variable.references.every(
          reference =>
            identifierToSurroundingFunctionMap.get(reference.identifier) ===
            surroundingFunction
        ) &&
        variable.defs.every(
          def =>
            identifierToSurroundingFunctionMap.get(def.name) ===
            surroundingFunction
        )
      );
    }
    ...
    function findOutdatedReads(
      ...
      expressionsByCodePathSegment
        .get(codePathSegment)
        .forEach(({ entering, node }) => {
    ...
    return {
      ...
      "AssignmentExpression, Identifier, MemberExpression, AwaitExpression, YieldExpression"(
        node
      ) {
        expressionsByCodePathSegment
          .get(currentCodePathSegment)
          .push({ entering: true, node });
      },

      "AssignmentExpression, Identifier, MemberExpression, AwaitExpression, YieldExpression:exit"(
        node
      ) {
        expressionsByCodePathSegment
          .get(currentCodePathSegment)
          .push({ entering: false, node });
      },
      ...
      Identifier(node) {
        if (functionStack.length) {
          identifierToSurroundingFunctionMap.set(
            node,
