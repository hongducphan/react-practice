...
function processText(
  ...
  let filePath, fileExtension, processor;
  ...
  for (const plugin in loadedPlugins) {
    if (
      loadedPlugins[plugin].processors &&
      Object.keys(loadedPlugins[plugin].processors).indexOf(fileExtension) >= 0
    ) {
  ...
  const autofixingEnabled =
    typeof fix !== "undefined" && (!processor || processor.supportsAutofix);
  ...
  const fixedResult = linter.verifyAndFix(text, config, {
    ...
    preprocess:
      processor &&
      (rawText => processor.preprocess(rawText, effectiveFilename)),
    postprocess:
      processor &&
      (problemLists => processor.postprocess(problemLists, effectiveFilename))
  });
...
function createIgnoreResult(filePath, baseDir) {
  ...
  const isInBowerComponents =
    baseDir && path.relative(baseDir, filePath).startsWith("bower_components");
...
class CLIEngine {
  ...
  constructor(providedOptions) {
    ...
    if (this.options.cache) {
      const cacheFile = getCacheFile(
        this.options.cacheLocation || this.options.cacheFile,
        this.options.cwd
      );
  ...
  resolveFileGlobPatterns(patterns) {
    return globUtils.resolveFileGlobPatterns(
  ...
  executeOnFiles(patterns) {
    ...
    const results = fileList.map(fileInfo => {
      ...
      if (options.cache) {
        const cachedLintResults = lintResultCache.getCachedLintResults(
          fileInfo.filename
        );

        if (cachedLintResults) {
          const resultHadMessages =
            cachedLintResults.messages && cachedLintResults.messages.length;

          if (resultHadMessages && options.fix) {
            debug(
              `Reprocessing cached file to allow autofix: ${fileInfo.filename}`
            );
          } else {
            debug(
              `Skipping file since it hasn't changed: ${fileInfo.filename}`
            );
