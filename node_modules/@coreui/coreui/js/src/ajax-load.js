...
const AjaxLoad = ($ => {
  ...
  class AjaxLoad {
    ...
    loadPage(url) {
      ...
      $.ajax({
        ...
        success(result) {
          ...
          const scripts = Array.from(wrapper.querySelectorAll("script")).map(
            script => script.attributes.getNamedItem("src").nodeValue
          );
    ...
    setUpUrl(url) {
      ...
      $(
        `${Selector.NAV_DROPDOWN}:has(a[href="${
          url.replace(/^\//, "").split("?")[0]
        }"])`
      ).addClass(ClassName.OPEN);
      $(
        `${Selector.NAV_ITEM} a[href="${url.replace(/^\//, "").split("?")[0]}"]`
