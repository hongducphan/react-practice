...
function itDefaultHandler(text, method) {
  try {
    ...
    if (err instanceof assert.AssertionError) {
      err.message += ` (${util.inspect(err.actual)} ${
        err.operator
      } ${util.inspect(err.expected)})`;
    }
    throw err;
  }
}
    ...
    function testInvalidTemplate(item) {
      ...
      if (typeof item.errors === "number") {
        ...
        for (let i = 0, l = item.errors.length; i < l; i++) {
          ...
          assert(
            !message.fatal,
            `A fatal parsing error occurred: ${message.message}`
          );
          assert(
          ...
          if (typeof error === "string" || error instanceof RegExp) {
            ...
            if (hasOwnProperty(error, "message")) {
              ...
              if (!hasOwnProperty(rule.meta.messages, error.messageId)) {
                const friendlyIDList = `[${Object.keys(rule.meta.messages)
                  .map(key => `'${key}'`)
                  .join(", ")}]`;
                ...
                assert(
                  ...
                  `Invalid messageId '${
                    error.messageId
                  }'. Expected one of ${friendlyIDList}.`
                );
              }
              assert.strictEqual(
                error.messageId,
                ...
                `messageId '${
                  message.messageId
                }' does not match expected messageId '${error.messageId}'.`
              );
              if (hasOwnProperty(error, "data")) {
                ...
                assert.strictEqual(
                  message.message,
                  rehydratedMessage,
                  `Hydrated message "${rehydratedMessage}" does not match "${
                    message.message
                  }"`
                );
              }
