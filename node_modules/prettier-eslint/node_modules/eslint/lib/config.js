...
class Config {
  ...
  constructor(providedOptions, linterContext) {
    ...
    this.configCache.setMergedVectorConfig(
      this.baseConfig.filePath,
      this.baseConfig
    );
  ...
  getLocalConfigHierarchy(directory) {
    ...
    for (const localConfigFile of localConfigFiles) {
      ...
      const localConfigHierarchyCache = this.configCache.getHierarchyLocalConfigs(
        localConfigDirectory
      );

      if (localConfigHierarchyCache) {
        const localConfigHierarchy = localConfigHierarchyCache.concat(
          configs.reverse()
        );

        this.configCache.setHierarchyLocalConfigs(
          searched,
          localConfigHierarchy
        );
       ...
       */
      if (
        localConfigDirectory === PERSONAL_CONFIG_DIR &&
        localConfigFile !== projectConfigPath
      ) {
        continue;
      }
  ...
  getConfig(filePath) {
    ...
    config = ConfigOps.applyEnvironments(
      config,
