...
class CLIEngine {
  ...
  static outputFixes(report) {
    report.results
      .filter(result => result.hasOwnProperty("output"))
      .forEach(result => {
  ...
  executeOnFiles(patterns) {
    ...
    function hashOfConfigFor(filename) {
      ...
      if (!configHashCache.has(config)) {
        configHashCache.set(
          config,
          hash(`${pkg.version}_${stringify(config)}`)
        );
      }
    ...
    const fileList = globUtil.listFilesToProcess(
      this.resolveFileGlobPatterns(patterns),
      options
    );
    const results = fileList.map(fileInfo => {
      ...
      if (options.cache) {
        ...
        const changed =
          descriptor.changed || descriptor.meta.hashOfConfig !== hashOfConfig;
