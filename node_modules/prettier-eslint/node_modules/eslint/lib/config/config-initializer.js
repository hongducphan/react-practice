...
const util = require("util"),
  ...
  getSourceCodeOfFiles = require("../util/source-code-util")
...
function installModules(config, installESLint) {
  ...
  // Install packages
  const modulesToInstall = Object.keys(modules).map(
...
function configureRules(answers, config) {
  ...
  try {
    sourceCodes = getSourceCodeOfFiles(
      patterns,
  ...
  return inquirer
    .prompt([
      ...
      },
      {
        ...
        when(answers) {
          return (
            (answers.source === "guide" && answers.packageJsonExists) ||
            answers.source === "auto"
          );
        }
      },
      {
        ...
        message(answers) {
          ...
          return `The style guide "${answers.styleguide}" requires eslint@${
            answers.requiredESLintVersionRange
          }. You are currently using eslint@${
            answers.localESLintVersion
          }.\n  Do you want to ${verb}?`;
        },
        default: true,
    ...
    .then(earlyAnswers => {
      ...
      if (earlyAnswers.source === "guide") {
        ...
        if (
          ...
          log.info(
            `Note: it might not work since ESLint's version is mismatched with the ${
              earlyAnswers.styleguide
            } config.`
          );
        }
        ...
        const config = getConfigForStyleGuide(
          earlyAnswers.styleguide,
          earlyAnswers.installESLint
        );
