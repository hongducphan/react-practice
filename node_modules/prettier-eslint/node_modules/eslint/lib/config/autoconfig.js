...
class Registry {
  ...
  buildRuleSets() {
    ...
    const addRuleToRuleSet = function(rule) {
      ...
      if (
        this.rules[rule][idx] &&
        (hasFewCombos || this.rules[rule][idx].specificity <= 2)
      ) {
        /*
        ...
        if (
          !hasFewCombos &&
          typeof this.rules[rule][idx].config[1] === "object"
        ) {
          return;
        }
  ...
  stripFailingConfigs() {
    ...
    ruleIds.forEach(ruleId => {
      const errorFreeItems = newRegistry.rules[ruleId].filter(
        registryItem => registryItem.errorCount === 0
      );
  ...
  stripExtraConfigs() {
    ...
    newRegistry.rules = Object.assign({}, this.rules);
    ruleIds.forEach(ruleId => {
      newRegistry.rules[ruleId] = newRegistry.rules[ruleId].filter(
        registryItem => typeof registryItem.errorCount !== "undefined"
      );
    });
  ...
  getFailingRulesRegistry() {
    ...
    ruleIds.forEach(ruleId => {
      const failingConfigs = this.rules[ruleId].filter(
        registryItem => registryItem.errorCount > 0
      );

      if (
        failingConfigs &&
        failingConfigs.length === this.rules[ruleId].length
      ) {
  ...
  filterBySpecificity(specificity) {
    ...
    newRegistry.rules = Object.assign({}, this.rules);
    ruleIds.forEach(ruleId => {
      newRegistry.rules[ruleId] = this.rules[ruleId].filter(
        registryItem => registryItem.specificity === specificity
      );
    });
