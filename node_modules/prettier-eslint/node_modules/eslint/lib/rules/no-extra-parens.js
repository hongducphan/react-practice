  ...
  create(context) {
    ...
    function checkCallNew(node) {
      ...
      if (hasExcessParens(callee) && precedence(callee) >= precedence(node)) {
        const hasNewParensException =
          callee.type === "NewExpression" && !isNewExpressionWithParens(callee);
      ...
      if (node.arguments.length === 1) {
        if (
          hasDoubleExcessParens(node.arguments[0]) &&
          precedence(node.arguments[0]) >= PRECEDENCE_OF_ASSIGNMENT_EXPR
        ) {
        ...
        node.arguments
          .filter(
            arg =>
              hasExcessParens(arg) &&
              precedence(arg) >= PRECEDENCE_OF_ASSIGNMENT_EXPR
          )
          .forEach(report);
      }
    }
    ...
    function checkSpreadOperator(node) {
      const hasExtraParens =
        precedence(node.argument) >= PRECEDENCE_OF_ASSIGNMENT_EXPR
          ? hasExcessParens(node.argument)
    ...
    return {
        ...
        node.elements
          .filter(
            e =>
              e &&
              hasExcessParens(e) &&
              precedence(e) >= PRECEDENCE_OF_ASSIGNMENT_EXPR
          )
          .forEach(report);
      },
      ...
      ArrowFunctionExpression(node) {
        ...
        if (node.body.type !== "BlockStatement") {
          ...
          if (
            ...
            precedence(node.body) >= PRECEDENCE_OF_ASSIGNMENT_EXPR
          ) {
            report(node.body);
          }
        }
      ...
      ConditionalExpression(node) {
        ...
        if (
          hasExcessParens(node.consequent) &&
          precedence(node.consequent) >= PRECEDENCE_OF_ASSIGNMENT_EXPR
        ) {
          report(node.consequent);
        }

        if (
          hasExcessParens(node.alternate) &&
          precedence(node.alternate) >= PRECEDENCE_OF_ASSIGNMENT_EXPR
        ) {
        ...
        node.properties
          .filter(property => {
            ...
            return (
              ...
              precedence(value) >= PRECEDENCE_OF_ASSIGNMENT_EXPR
            );
          })
