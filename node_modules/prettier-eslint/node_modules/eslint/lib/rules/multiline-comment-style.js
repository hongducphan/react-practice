...
module.exports = {
  ...
  create(context) {
    ...
    function convertToStarredBlock(firstComment, commentLinesList) {
      ...
      const starredLines = commentLinesList.map(
        line => `${initialOffset} *${line}`
      );
        ...
        if (commentGroup.length > 1) {
          context.report({
            ...
            fix(fixer) {
              ...
              const starredBlock = `/*${convertToStarredBlock(
                commentGroup[0],
                commentLines
              )}*/`;
          ...
          const expectedLinePrefix = `${sourceCode.text.slice(
            block.range[0] - block.loc.start.column,
            block.range[0]
          )} *`;

          if (!/^\*?\s*$/.test(lines[0])) {
            const start = block.value.startsWith("*")
              ? block.range[0] + 1
              : block.range[0];
          ...
          for (
            let lineNumber = block.loc.start.line + 1;
            lineNumber <= block.loc.end.line;
            lineNumber++
          ) {
            ...
            if (!lineText.startsWith(expectedLinePrefix)) {
              context.report({
                ...
                fix(fixer) {
                  ...
                  const linePrefixLength = lineText.match(/^\s*\*? ?/)[0]
                    .length;
                  const commentStartIndex = lineStartIndex + linePrefixLength;

                  const replacementText =
                    lineNumber === block.loc.end.line ||
                    lineText.length === linePrefixLength
                      ? expectedLinePrefix
                      : `${expectedLinePrefix} `;
