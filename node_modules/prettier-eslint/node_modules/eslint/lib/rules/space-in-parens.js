  ...
  create(context) {
    ...
    function isOpenerException(token) {
      return (
        ...
        exceptions.openers.indexOf(token.value) >= 0
      );
    }
    ...
    function isCloserException(token) {
      return (
        token.type === "Punctuator" &&
        exceptions.closers.indexOf(token.value) >= 0
      );
    }
      ...
      Program: function checkParenSpaces(node) {
        ...
        tokens.forEach((token, i) => {
          ...
          if (token.value === "(" && shouldOpenerHaveSpace(token, nextToken)) {
            ...
            token.value === "(" &&
            shouldOpenerRejectSpace(token, nextToken)
          ) {
            context.report({
              node,
              loc: token.loc.start,
            ...
            token.value === ")" &&
            shouldCloserHaveSpace(prevToken, token)
          ) {
            ...
            token.value === ")" &&
            shouldCloserRejectSpace(prevToken, token)
          ) {
            context.report({
              node,
              loc: token.loc.start,
