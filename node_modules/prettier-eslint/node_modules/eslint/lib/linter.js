  ...
  try {
    ...
    return {
      ...
      error: {
        ...
        severity: 2,
        source: null,
        message: `Failed to parse JSON from '${normalizedString}': ${
          ex.message
        }`,
  ...
  Object.keys(commentDirectives.enabledGlobals).forEach(name => {
    ...
    if (!variable) {
      ...
      variable.eslintExplicitGlobalComment =
        commentDirectives.enabledGlobals[name].comment;
      globalScope.variables.push(variable);
      globalScope.set.set(name, variable);
    }
...
function getDirectiveComments(filename, ast, ruleMapper) {
  ...
  ast.comments
    ...
    .forEach(comment => {
      ...
      const match = /^(eslint(-\w+){0,3}|exported|globals?)(\s|$)/.exec(
        trimmedCommentText
      );
      ...
      if (
        /^eslint-disable-(next-)?line$/.test(match[1]) &&
...
function normalizeVerifyOptions(providedOptions) {
  ...
  const providedFilename = isObjectOptions
    ? providedOptions.filename
    : providedOptions;

  return {
    filename:
      typeof providedFilename === "string" ? providedFilename : "<input>",
    allowInlineConfig:
      !isObjectOptions || providedOptions.allowInlineConfig !== false,
    reportUnusedDisableDirectives:
      isObjectOptions && !!providedOptions.reportUnusedDisableDirectives
  };
}
...
function resolveParserOptions(providedOptions, enabledEnvironments) {
  const parserOptionsFromEnv = enabledEnvironments
    ...
    .reduce(
      (parserOptions, env) => ConfigOps.merge(parserOptions, env.parserOptions),
      {}
    );

  const mergedParserOptions = ConfigOps.merge(
...
function parse(text, providedParserOptions, parserName, parserMap, filePath) {
  ...
  try {
    ...
    const scopeManager =
      parseResult.scopeManager || analyzeScope(ast, parserOptions, visitorKeys);
    ...
    const message = `Parsing error: ${ex.message
      .replace(/^line \d+:/i, "")
      .trim()}`;
...
function getScope(scopeManager, currentNode, ecmaVersion) {
  ...
  if (
    ...
    ].indexOf(currentNode.type) >= 0 ||
    (ecmaVersion >= 6 &&
      ["BlockStatement", "SwitchStatement", "CatchClause"].indexOf(
        currentNode.type
      ) >= 0)
  ) {
...
function runRules(
  ...
  const sharedTraversalContext = Object.freeze(
    Object.assign(Object.create(BASE_TRAVERSAL_CONTEXT), {
      ...
      getDeclaredVariables: sourceCode.scopeManager.getDeclaredVariables.bind(
    ...
    const ruleContext = Object.freeze(
      Object.assign(Object.create(sharedTraversalContext), {
        ...
        report() {
           ...
           * https://github.com/eslint/eslint/issues/9193
           */
          sharedTraversalContext._linter.report(
...
module.exports = class Linter {
  ...
  _verifyWithoutProcessors(
    ...
    const commentDirectives = options.allowInlineConfig
      ? getDirectiveComments(options.filename, sourceCode.ast, ruleId =>
  ...
  verify(textOrSourceCode, config, filenameOrOptions) {
    const preprocess =
      (filenameOrOptions && filenameOrOptions.preprocess) ||
      (rawText => [rawText]);
    const postprocess =
      (filenameOrOptions && filenameOrOptions.postprocess) || lodash.flatten;

    return postprocess(
      preprocess(textOrSourceCode).map(textBlock =>
        this._verifyWithoutProcessors(textBlock, config, filenameOrOptions)
      )
    );
  }
